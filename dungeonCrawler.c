//#define SDL_MAIN_HANDLED // use if main() not pass argument
#include <SDL.h>
#include "dungeon.h"
...
int main(int argc, char **argv) // use SDL_MAIN_HANDLED if int main() not pass arguments: int argc, char **argv
{
    ...
    while (window_opened)
    {
        ...
        if (receive_len > 0)
        {
            ...
            if (!is_joined)
            {
                ...
                message_type = pointer[0];
                message_id = pointer[1];
                player_id = pointer[2];

                if (message_type == MESSAGE_WELCOME)
                {
                    printf("####################################\n");
                    printf("Welcome [player_%d]! Get ready for action!\n", player_id);
                    printf("###############\n");
                    printf("Reply ACK sent!\n");
                    printf("###############\n");
                    ...
                    // Send packet
                    send_packet(s, MESSAGE_ACK, message_id, player_id, 0, 0, &sin);

                    // Set client state to joined
                    is_joined = 1;

                    // Change message type
                    message_type = MESSAGE_POSITION;
                }
            }
            else
            {
                ...
                if (packet->message_type == MESSAGE_POSITION)
                {
                    ...
                    for (size_t i = 0; i < current_players; i++)
                    {
                        ...
                        {
                            if (other_players[i].last_order < packet->message_id)
                            {
                                // Delta
                                ...
                                other_players[i].last_order = packet->message_id;
                            }
                ...
                else if (packet->message_type == MESSAGE_MELEE)
                {
                    if (player_id == packet->player_id)
                    {
                        printf("####################################\n");
                        printf("You used melee attack!\n");

                        // Send packet
                        send_packet(s, MESSAGE_ACK, packet->message_id, player_id, 0, 0, &sin);
                    }
                    else
                    {
                        for (size_t i = 0; i < current_players; i++)
                        {
                            if (other_players[i].player_id == packet->player_id)
                            {
                                ...
                                {
                                    printf("####################################\n");
                                    printf("Player: %u has used melee attack!\n", other_players[i].player_id);
                                }
                            }
                        }
                    }
        ...
        if (timer_packet <= 0)
        {
            ...
            // Send packet
            send_packet(s, message_type, message_id, player_id, first.x, first.y, &sin);
